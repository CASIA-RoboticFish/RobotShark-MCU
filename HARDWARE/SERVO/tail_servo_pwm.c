...
/********************************************************************************
*
* Function name ：tail_pwm_set_compare
* Description   ：尾鳍舵机初始化
*                 
* Parameter     ：
...
void tail_pwm_set_compare( uint16_t compare_value, uint8_t channel )
{
	...
	{
		case TAIL_SERVO1:
			TIM_SetCompare4(TIM4, compare_value);// 有一条鱼搞错了，所以这里改一下，需要的话改回来
			break;
		case TAIL_SERVO2:
...
void tail_servo_init ( void )
{
  ...
  pwm_tim4_init(20000, 84);
	tail_pwm_set_compare(TAIL_SERVO1_MID_VALUE,TAIL_SERVO1);
	tail_pwm_set_compare(TAIL_SERVO2_MID_VALUE,TAIL_SERVO2);
	tail_pwm_set_compare(TAIL_SERVO3_MID_VALUE,TAIL_SERVO3);
	tail_pwm_set_compare(TAIL_SERVO4_MID_VALUE,TAIL_SERVO4);
}
