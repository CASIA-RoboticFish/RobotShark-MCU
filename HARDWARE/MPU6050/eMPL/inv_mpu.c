...
struct hw_s hw={
  //0x69,	 //addr // 这里改地址,ZPF:如果是板载IMU
	0x68,	 //addr // 这里改地址,ZPF:如果是外接IMU
  1024,	 //max_fifo
  118,	 //num_reg
  340,	 //temp_sens
  -521,	 //temp_offset
...
const struct hw_s hw = {
    //.addr           = 0x69, // 这里改地址,ZPF:如果是板载IMU
		.addr           = 0x68, // 这里改地址,ZPF:如果是外接IMU
    .max_fifo       = 1024,
    .num_reg        = 128,
    .temp_sens      = 321,
    .temp_offset    = 0,
...
为获取无零偏的数据
********************************/
static long g_gyrobias[3] = {0.0f, 0.0f, 0.0f};
static long g_accelbias[3] = {0.0f, 0.0f, 0.0f};
...
u8 run_self_test(void)
{
	...
	if (result == 0x3) 
	{
		...
		*/
		...
		accel[1] *= accel_sens;
		accel[2] *= accel_sens;
		g_accelbias[0] = accel[0];
		g_accelbias[1] = accel[1];
		g_accelbias[2] = accel[2];
...
//	{
//		int i = dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors,&more);
//		if(i==-1)
//			return 1;
//		if(i==-2)
//			return 2;
...
int mpu_get_gyronobias_reg(short *data, unsigned long *timestamp)
{
    unsigned char tmp[6];
    ...
    if (i2c_read(st.hw->addr, st.reg->raw_gyro, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
		data[0] -= (short)g_gyrobias[0];
    data[1] = (tmp[2] << 8) | tmp[3];
		data[1]	-= (short)g_gyrobias[1];
    data[2] = (tmp[4] << 8) | tmp[5];
		data[2]	-= (short)g_gyrobias[2];
    if (timestamp)
        get_ms(timestamp);
    return 0;
}
...
int mpu_get_accelnobias_reg(short *data, unsigned long *timestamp)
{
    unsigned char tmp[6];
    ...
    if (i2c_read(st.hw->addr, st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
		data[0] -= (short)g_accelbias[0];
    data[1] = (tmp[2] << 8) | tmp[3]; 
		data[1] -= (short)g_accelbias[1];
    data[2] = (tmp[4] << 8) | tmp[5]; 
		data[2] -= (short)g_accelbias[2];
    if (timestamp)
        get_ms(timestamp);
    return 0;
}
...
int mpu_set_address(uint8_t mpu_addr)
{
	if(mpu_addr == 0x68)      // 外接IMU
	{	
		hw.addr = 0x68;
	}
	else if(mpu_addr == 0x69) // 板载IMU
	{
		hw.addr = 0x69;
	}
	else
...
int mpu_set_orientation(uint8_t mpu_addr)
{
	if(mpu_addr == 0x68)      // 外接IMU
	{	
	...
	{
		gyro_orientation[0] = 0;
		gyro_orientation[1] = 0;
		gyro_orientation[2] = 1;
		gyro_orientation[3] = 0;
		gyro_orientation[4] = 1;
		gyro_orientation[5] = 0;
		gyro_orientation[6] = -1;
		gyro_orientation[7] = 0;
		gyro_orientation[8] = 0;
	}
	else
	{
		return 1;
